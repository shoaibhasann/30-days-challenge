const apiKey = "uXPo7t4EVO5UT4E1G0jsED9ZsIQGYJXe";
...
async function getCurrentWeather(locationKey) {
  try {
    ...
    const precipitation = weatherDetails.PrecipitationSummary.Past12Hours.Metric.Value;
    ...
    return { temperature, feelsLike, weatherText, 
           iconNumber, humidity, dewPoint, visibility, windSpeed, precipitation };
...
async function getHourlyForecast(locationKey){
   const url = `http://dataservice.accuweather.com/forecasts/v1/hourly/12hour/${locationKey}?apikey=${apiKey}&metric=true`;
    try {
      ...
      console.error(
        "Error while fetching hourly forecast data:",
        error
      );
    }
}
...
export { getLocationKey, getCurrentWeather, autoCompleteSearch, getHourlyForecast, getDailyForecast };
